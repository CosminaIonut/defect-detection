...
def load_all_models(n_models):
    ...
    for i in range(n_models):
        ...
        filename = '../../trained_models/severity/models__adam_0.09702322502562324LR_[39]HN_32BS_10P_val_mseM_200epochs/model_' + str(i + 1) + '.h5'
...
def read_test_data(path):
    ...
    y_test_input = df_samples_slim.iloc[:, np.r_[0, 1]].values
...
#     train_X_list[i], test_X_list[i], train_Y_list[i], test_Y_list[i] = split_data(data_no_overlap[i],test_size=0.30)
...
# savetxt('train_test_csv/train_no_overlap_severity_80%.csv', X_train, delimiter=',')
# savetxt('train_test_csv/train_target_no_overlap_severity_80%.csv', Y_train, delimiter=',')
# savetxt('train_test_csv/test_target_no_overlap_severity_20%.csv', Y_test, delimiter=',')
# savetxt('train_test_csv/test_no_overlap_severity_20%.csv', X_test, delimiter=',')
#
#
...
#     plt.plot(i, y_pred[i][1], 'x', color='blue')
#     plt.plot(i, y_pred[i][0], 'o', color='red')
#     plt.plot(i, Y_test[i][1], 'x', color='green')
#     plt.plot(i, Y_test[i][0], 'o', color='green')
# error_severity = mean_squared_error([i[1] for i in Y_test], [i[1] for i in y_pred])
# error_position = mean_squared_error([i[0] for i in Y_test], [i[0] for i in y_pred])
# error = mean_squared_error(Y_test, y_pred)
# print("Error(MSE) Position",format(float(error_position), '.7f'))
# print("Error(MSE) Severity",format(float(error_severity), '.7f'))
# print("Error(MSE)",format(float(error), '.7f'))
#
#
# # # Save plots
# plt.savefig('../../plots/severity/test.png')
#
#
# # # Write predictions in a text file
# f = open("../../predictions/severity/test.txt", "w")
#
# for x in y_pred:
